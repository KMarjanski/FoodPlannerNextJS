...
import { usePlannerLastModified } from "./usePlannerLastModified"
...
export function MealPlannerDashboard() {
  ...
  // Ustaw aktywny tydzień po załadowaniu mealData, jeśli obecny nie istnieje
  useEffect(() => {
    if (mealData.length > 0) {
      const exists = mealData.some((w) => w.id === activeWeek)
      if (!exists) {
        setActiveWeek(mealData[0].id)
      }
    }
  }, [mealData])
  ...
  const [plannerLastModified, refetchLastModified] = usePlannerLastModified();
  const todayWeekIndex = useMemo(() => {
    if (!displayedWeeks.length || !plannerLastModified) {
      return -1;
    }
    const lastMod = new Date(plannerLastModified);
    const now = new Date();
    lastMod.setHours(0,0,0,0);
    now.setHours(0,0,0,0);
    const diffDays = Math.floor((now.getTime() - lastMod.getTime()) / (1000 * 60 * 60 * 24));
    const weekIdx = Math.floor(diffDays / 7);
    console.log("[Planner] Debug badge Dzisiaj", {
      lastModified: plannerLastModified,
    ...
    return weekIdx;
  }, [displayedWeeks, plannerLastModified]);

  // Reset active week if it no longer exists lub zmienia się todayWeekIndex (mobile)
  // Ustaw activeWeek na tydzień "Dzisiaj" tylko przy pierwszym załadowaniu lub jeśli activeWeek nie istnieje
  useEffect(() => {
    if (!displayedWeeks.length) return;
    // Jeśli activeWeek nie istnieje w displayedWeeks, ustaw na todayWeekIndex lub pierwszy tydzień
    if (!displayedWeeks.find((w) => w.id === activeWeek)) {
      if (isMobile && todayWeekIndex >= 0 && todayWeekIndex < displayedWeeks.length) {
        setActiveWeek(displayedWeeks[todayWeekIndex].id);
      } else {
        setActiveWeek(displayedWeeks[0].id);
      }
    }
    // W przeciwnym razie nie nadpisuj wyboru użytkownika
  }, [displayedWeeks, todayWeekIndex, isMobile]);
